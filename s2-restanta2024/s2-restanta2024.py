...
#2
merged=pd.merge(df_coduri, df_netflix, on='Cod')
merged=merged.groupby("Continent")[coloane].agg(['mean', 'std'])
merged=merged.xs('std', axis=1, level=1)/merged.xs('mean', axis=1, level=1)
merged=merged.round(3).reset_index()
merged=merged.sort_values(by="Librarie", ascending=False)
merged.to_csv("Cerinta2.csv", index=False)
